...
ydl_opts = {
    ...
    'extractor_args': {
        'youtube': {
            'skip': ['po_token', 'webpage', 'configs'], # 叫它不要去抓任何 Token
        }
    },
}
...
async def on_message(message):
    ...
    if bot.user.mentioned_in(message) and "早安" in message.content:
        # 準備回話清單
        responses = [
            f"早安啊 {message.author.display_name}！今天也要元氣滿滿喔！☀️",
            "早安～你有沒有記得喝杯水呀？(,,・ω・,,)",
            "嗚哇...早安...我還想再睡五分鐘...（戳）",
            f"早安！看到 {message.author.display_name} 出現我就開心了 (ﾉ>ω<)ﾉ",
            "早安！今天的早餐打算吃什麼呢？🥪",
...
async def on_ready():
    ...
    # 檢查任務是否已在執行，避免重複啟動
    if not daily_ritual1.is_running():
        daily_ritual1.start()
    if not daily_ritual2.is_running():
        daily_ritual2.start()
    if not auto_reboot.is_running():
        auto_reboot.start()
